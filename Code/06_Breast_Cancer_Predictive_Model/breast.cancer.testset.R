...
merged.df.filt.targ = discovery.set[discovery.set$Sex == 'Female',]
...
if (dmrcalling == T) {
  ...
  #dds = readRDS('/.mounts/labs/awadallalab/private/ncheng/cfmedip_data/cptp_samples/fragmentation/aix13.updated1/all.sample.fragmentation//AIX13.dv.Allall.samples.dds.inserts.400.all.300.q20.RDS')
  ...
  dds = readRDS('/.mounts/labs/awadallalab/private/ncheng/cfmedip_data/cptp_samples/fragmentation/aix13.updated1/all.sample.fragmentation/regulatory.counts/aix13.all.dv.1000.genhancer.dds.RDS')
  ...
  merged.df.filt.targ = merged.df.filt.targ[merged.df.filt.targ$Sex == 'Female',]
  ...
  saveRDS(res.df,paste0(savedir1,'discovery.breast.allnorm.ntotalTB.genhancers.RDS'))
...
if (dmrcalling == T) {
  ...
  #saveRDS(res.df,paste0(savedir,'discovery.breast.nonage.adjusted.dmrs.genhancers.RDS'))
  #saveRDS(res.df,paste0(savedir1,'discovery.breast.femalenorm.ntotalTB.silencers.RDS'))
  saveRDS(res.df,paste0(savedir1,'discovery.prostate.allnorm.ntotal.genhancers.RDS'))
  
  
}
####feature weghting####

#comparing average frequency of each feature
setwd('/.mounts/labs/awadallalab/private/ncheng/cfmedip_data/cptp_samples/fragmentation/aix13.updated1/all.sample.fragmentation/breast.cancer.cv/regulatory.regions.v10.newsplit5/genhancer.1000.ntotalTB.allnorm.v3/predictions.bm1.hyper.0.25')
weight.files = list.files(pattern= '.*weights.RDS')
perf.df= readRDS('/.mounts/labs/awadallalab/private/ncheng/cfmedip_data/cptp_samples/fragmentation/aix13.updated1/all.sample.fragmentation/breast.cancer.cv/regulatory.regions.v10.newsplit5//mean.cv.genhancer.performance.newsplit.RDS')
top.perf = perf.df[perf.df$All == max(perf.df$All),][1,]
weight.list = lapply(weight.files,function(x) {
  tmp = readRDS(x)
  #top.perf = overall.perf.list.top[[2]][1,]
  overall.perf.targ.breast = tmp#[tmp$covariate == 'None',]
  parameters = c('mode','comparison','test','n.features','mat','covariate','model')
  parameters.filt =parameters[parameters %in% colnames(tmp)]
  #dir.filt = gsub('covar.base.',direction)
  for (p in parameters.filt) {
    if (p != 'direction') {
      overall.perf.targ.breast = overall.perf.targ.breast[overall.perf.targ.breast[,p] == top.perf[,p],]
      
    } else{
      overall.perf.targ.breast = overall.perf.targ.breast[overall.perf.targ.breast[,p] == gsub('.*\\.','',top.perf[,p]),]
      
    }
    print(nrow(overall.perf.targ.breast))
    
  }
  return(overall.perf.targ.breast)
} )

weights.df = do.call('rbind',weight.list)
weights.df = weights.df[weights.df$coef != 0 ,]
weights.df.filt = weights.df[,c('PC','coef')]

#if ordering by frequency
window.freq =data.frame(table(weights.df.filt[,1]))
res.df.targ = window.freq[order(-window.freq$Freq),]
colnames(res.df.targ) = c('window','freq')
res.df.targ$window =gsub('`','',res.df.targ$window)
res.df.targ = res.df.targ[-1,]
rownames(res.df.targ) =res.df.targ$window

#if ordering by average weighting
weight.all = weights.df.filt
res.df.targ = ddply(weight.all,'PC',numcolwise(mean))
colnames(res.df.targ) = c('window','coef')
res.df.targ$window =gsub('`','',res.df.targ$window)
res.df.targ = res.df.targ[-1,]
rownames(res.df.targ) =res.df.targ$window
...
for (s in sf[2]) {
  for (m in matrices[2]) {
    ...
    if (m == 'Female.1000') {
      ...
      dds = readRDS(paste0('/.mounts/labs/awadallalab/private/ncheng/cfmedip_data/cptp_samples/fragmentation/aix13.updated1/all.sample.fragmentation/regulatory.counts/aix13.all.dv.1000.genhancer.dds.RDS'))
    ...
    } else if (m == 'Female.400') {
      sample.matrix = readRDS('/.mounts/labs/awadallalab/private/ncheng/cfmedip_data/cptp_samples/fragmentation/aix13.updated1/all.sample.fragmentation//AIX13.allbg.dv.Femaleall.samples.deseq.normcounts.inserts.400.all.300.q20.RDS')
      
    } else if (m == 'All.400') {
      sample.matrix = readRDS('/.mounts/labs/awadallalab/private/ncheng/cfmedip_data/cptp_samples/fragmentation/aix13.updated1/all.sample.fragmentation//AIX13.dv.Allall.samples.deseq.normcounts.inserts.400.all.300.q20.RDS')
      
    }
    fc.cutoff = c(0,0.1,0.15,0.2,0.25,0.3,0.35,0.4,0.45)
    fc.cutoff=c(0,0.05,0.1,0.15,0.2,0.25,0.3,0.35,0.4)#[seedno] #c(0,0.25,0.45)
    #fc.cutoff=c(0,0.1,0.4)
    fc.cutoff=0.25
    results.df.all = NULL
    print('c2')
    for (fc in fc.cutoff) {
      for (d in directions) {
        ...
        savedir2=paste0(savedir1,'/fc.assess.ntotalTB.genhancer.final/')
        ...
        saveRDS(results.df.all[results.df.all$model =='logreg.old.alpha1',],paste0(savedir2,'validaiton.performance.RDS'))
...
for (s in sf[2]) {
  for (m in matrices[2]) {
    ...
    #new.split
    #res.df = readRDS(paste0(savedir1,'discovery.allbg.Breast.',m,'.',s,'.dmrs.RDS'))
    res.df = readRDS(paste0(savedir1,'discovery.breast.allnorm.ntotalTB.silencers.RDS'))
    res.df$window =rownames(res.df)
    directions = c('abs','hyper')[1:2]#[2]#[1]#[foldno]#[1]#[2]
    print('c1')
    if (m == 'Female.1000') {
      #sample.matrix = readRDS('/.mounts/labs/awadallalab/private/ncheng/cfmedip_data/cptp_samples/fragmentation/aix13.updated1/all.sample.fragmentation/regulatory.counts/aix13.all.dv.1000.genhancer.norm.counts.RDS')
    ...
    for (fc in fc.cutoff) {
      for (d in directions[2]) {
        ...
        res.df.targ = res.df#[rownames(res.df) %in% background,]
        if (grepl('abs',d) == T) {
          res.df.targ = res.df.targ[which(abs(res.df.targ$log2FoldChange) > fc & res.df.targ$baseMean > 0),]
        } else if (grepl('hyper',d) == T){
          res.df.targ = res.df.targ[which(res.df.targ$log2FoldChange> fc   & res.df.targ$baseMean > 0),]
        }
        print('c3')
        res.df.targ = res.df.targ[order(res.df.targ$pvalue),]
        ...
        #base
        #n.features = c(seq(10,100,10),seq(125,300,25))
        n.features = c(seq(50,150,10))#,seq(250,400,50))
        #n.features = c(75,100)
        n.features = n.features[n.features <= nrow(res.df.targ)]
        #n.features = c(25,50)
        print('c5')
        ...
        #results.df.all$fc = fc
        savedir2=paste0(savedir1,'/fc.assess.ntotal.genhancer.v2/')
        dir.create(savedir2,recursive = T)
        saveRDS(results.df.all,paste0(savedir2,'discovery.Breast.',filt,'.',fc,'.',d,'.',s,'.',m,'.performance.covarbase.RDS'))
...
####combining enhancer and silnecer####
results.df.all = NULL
matrices = c('Female.1000','All.1000','Female.400','All.400')
...
for (s in sf[2]) {
  for (m in matrices[2]) {
    ...
    #res.df = readRDS(paste0(savedir1,'discovery.allbg.Breast.',m,'.',s,'.dmrs.RDS'))
    res.df = readRDS(paste0(savedir1,'discovery.breast.allnorm.ntotalTB.genhancers.RDS'))
    res.df1 = readRDS(paste0(savedir1,'discovery.breast.allnorm.ntotalTB.silencers.RDS'))
    
    rownames(res.df) = paste0('genhancer_',rownames(res.df))
    rownames(res.df1) = paste0('silencer_',rownames(res.df1))
    res.df = rbind(res.df,res.df1)
    res.df$window =rownames(res.df)
    directions = c('abs','hyper')[1]#[2]#[2]#[1]#[foldno]#[1]#[2]
    print('c1')
    if (m == 'Female.1000') {
      ...
      dds = estimateSizeFactors(dds[windows,targ.samples$GRP_Id])
      dds$condition = dds$group
      sample.matrix1 = counts(dds, normalized = T)
      rownames(sample.matrix1) = paste0('genhancer_',rownames(sample.matrix1))
      #
      dds = readRDS(paste0('/.mounts/labs/awadallalab/private/ncheng/cfmedip_data/cptp_samples/fragmentation/aix13.updated1/all.sample.fragmentation/regulatory.counts/aix13.all.dv.1000.silencer.dds.RDS'))
      #dds.matrix = readRDS(paste0(wkdir,'aix13.combined.',inserts,'.',marker,'.norm.counts.RDS'))
      ...
      dds$condition = dds$group
      sample.matrix = counts(dds, normalized = T)
      rownames(sample.matrix) = paste0('silencer_',rownames(sample.matrix))
      
      sample.matrix = rbind(sample.matrix,sample.matrix1)
    ...
    for (fc in fc.cutoff) {
      for (d in directions) {
        ...
        print(d)
        res.df.targ = data.frame(res.df,check.names=F,stringsAsFactors=F)#[rownames(res.df) %in% background,]
        if (grepl('abs',d) == T) {
          res.df.targ = res.df.targ[which(abs(res.df.targ$log2FoldChange) > fc & res.df.targ$baseMean > 0),]
        } else if (grepl('hyper',d) == T){
          res.df.targ = res.df.targ[which(res.df.targ$log2FoldChange> fc   & res.df.targ$baseMean > 0),]
...
setwd('/.mounts/labs/awadallalab/private/ncheng/cfmedip_data/cptp_samples/fragmentation/aix13.updated1/all.sample.fragmentation/breast.cancer.cv/regulatory.regions.v10/validation.breast.test/new.split2/fc.assess.ntotalTB.genhancer')
#setwd('x/fc.assess')
targ.files = list.files(pattern='.*performance.*')[1:18]
targ.list = lapply(targ.files,readRDS)
q = lapply(targ.list, function(x) {
  return.df = x[order(-x$auroc),]
  #return.df$group =
  rownames(return.df) = NULL
  return(head(return.df))
} )

q =do.call('rbind',q)
q = q[order(-q$auroc),]
q2 =q[q$GRP_Id == 'AIX_0017',]
a = targ.list[[1]]
a = a[a$auroc > 0.66,]
a = a[a$covariate == 'none',]
...
results.df.all = readRDS('//.mounts/labs/awadallalab/private/ncheng/cfmedip_data/cptp_samples/fragmentation/aix13.updated1/all.sample.fragmentation/breast.cancer.cv/regulatory.regions.v10/validation.breast.hrsplit//genhancer.hrsplit.final//fc.assess.ntotalTB.genhancer.final//validaiton.performance.RDS')
figdir='/.mounts/labs/awadallalab/private/ncheng/cfmedip_data/cptp_samples/fragmentation/aix13.updated1/all.sample.fragmentation/breast.cancer.cv/regulatory.regions.v10/validation.breast.hrsplit//figures/'
dir.create(figdir)
pred.df.targ = results.df.all[results.df.all$features == 90,];
name = paste0(figdir,'brca.validation');
merged.df.all= validation.set;
sample.info = validation.set;

score.cutoff=0.40;
dx.all = T;
cutpoint.use =T;
gs.plot= T
#saveRDS(pred.df.targ,pa




#####ploting performance####
#top.perf = readRDS('/.mounts/labs/awadallalab/private/ncheng/cfmedip_data/cptp_samples/fragmentation/aix13.updated1/all.sample.fragmentation/breast.cancer.cv/regulatory.regions.v10/validation.breast.test//genhancer.validation//fc.assess.ntotal.genhancer.v3/discovery.Breast.validation.genhancer.final.performance.covarbase.RDS')


figdir = paste0(savedir,'/enhancer.validation.final.90/')
