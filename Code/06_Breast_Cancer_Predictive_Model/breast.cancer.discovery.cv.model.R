...
sample.info.filt = sample.info.filt[sample.info.filt$data.partition %in% c('Discovery') & sample.info.filt$Sex == 'Female',]
targets = list('Breast' = sample.info.filt)
...
for (fold in foldno){
  for (i in names(targets)) {
    start = Sys.time()
    dds = readRDS(paste0(wkdir,'/ohs.1000.gehancer.dds.RDS'))
    
    #selecting all female discovery set samples
...
dds.matrix = readRDS(paste0(wkdir,'ohs.1000.gehancer.norm.counts.RDS'))
...
for (fold in foldno) {
  if (file.exists(paste0(savedir,i,'.',seedno,'.',fold,'.dmr.RDS')) == T) {
    ...
    #selecting samples of interest (ie female breast cancer/controls)
    targ.samples = targets[[i]]
